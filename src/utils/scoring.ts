import type { Player, Game, ProcessedPlayer } from '../types';

export const UMA: number[] = [30, 10, -10, -30];
export const RETURN_SCORE: number = 25000;

export function computePoints(rawScore: number, rank: number): number {
...
export function rankPlayers(players: Player[]): RankedPlayer[] {
  return (players as (Player & { _index: number })[])
    .map((p, i) => ({ ...p, _index: i }))
    .sort((a, b) => {
...
export function processGame(game: Game): ProcessedPlayer[] {
  ...
  return ranked.map((p) => {
    ...
    let adjustment = 0;
    if (chomboCount > 0) {
      adjustment = p.chombo ? -30 + 10 * (chomboCount - 1) : 10 * chomboCount;
    }
    return {
      name: p.name,
      rawScore: p.rawScore,
